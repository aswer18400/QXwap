...
if [ -n "${API_BASE_URL:-}" ]; then
  echo "[extra] Probing API_BASE_URL health"
  if curl -fsS --max-time 20 "${API_BASE_URL%/}/health" >/tmp/preflight-api-health.txt; then
    echo "✅ API health ok (${API_BASE_URL%/}/health)"
  elif curl -fsS --max-time 20 "${API_BASE_URL%/}/api/health" >/tmp/preflight-api-health.txt; then
    echo "✅ API health ok (${API_BASE_URL%/}/api/health)"
  else
    echo "❌ API health failed for API_BASE_URL=${API_BASE_URL}"
    exit 3
  fi
fi
