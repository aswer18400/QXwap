...
//
//   // ใน App.jsx ตรง renderMain() เพิ่ม:
//   case "add": return <AddListingScreen currentUser={currentUser} onDone={() => nav("feed")} onBack={() => nav("feed")} showToast={showToast} />;
//
//   // ใน BottomNav เปลี่ยน tab "market" เป็น "add":
//   { key: "add", icon: "➕", label: "ลงของ" }
// ============================================================

import { useState, useRef, useCallback } from "react";
import { createListing, uploadListingImages } from "../lib/supabase";
...
export default function AddListingScreen({ currentUser, onDone, onBack, showToast }) {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      // 1. สร้าง listing ก่อน (ยังไม่มีรูป)
